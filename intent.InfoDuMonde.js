...
exports.default = function (state, actions) {
	...
	var match;
	for (var rule in Config.modules.InfoDuMonde.rules) {	 
		match = (0, _helpers.syntax)(state.sentence, Config.modules.InfoDuMonde.rules[rule]); 	
		if (match) break;
	}
	
	if (match) { // vérifié !
		if (state.debug) info('Intent InfoDuMonde, Syntax: true');
		state.isIntent = true;
